...
if confirm "${prompt}Run automated firewall?";then
    echo -e "${warn}Clearing existing rules..."
    iptables -F
    iptables -X
    # Configure inbound firewall states
    # Allow loopback
    
    echo -e "${good}Allowing all to ${blue}LOOPBACK${nocolor}" 
    iptables -A INPUT -i lo -j ACCEPT
    # Allow ping
    echo -e "${good}Allowing ${blue}PING (ICMP)${nocolor}" 
    ...
    OLDIFS=$IFS
    IFS=$'\n'
    echo -e "${good}Adding rules for existing listening sockets"
    ...
    echo -e "${good}Allowing in any related or established sessions"
